    ...
    public class UserService : IUserService
    {
        ...
        public async Task<Response> Login(LoginRequest request)
        {
            try
            {
                ...
                return new Response
                {
                    ...
                    message = "Success",
                    token = token,
                    expirationTime = DateTime.Now.AddHours(1).ToString("yyyy-MM-dd HH:mm:ss"),
                    role = role?.RoleName,
                };
            }
            catch (Exception ex)
        ...
        public async Task<(bool IsSuccess, string? ErrorMessage, Response? response)> Register(UserDto request)
        {
            try
            {
                ...
                string saveUserToFile = $"UserId: {newUser.Id}; Email: {newUser.Email}; Password: {passwordSave}; fullName: {newUser.FullName}";
                File.AppendAllText("fileUser.txt", saveUserToFile + Environment.NewLine);
                ...
                if (request.FileImage != null && request.FileImage.Length > 0)
                {
                    ...
                    var imageUrl = await UploadImageAsync(request.FileImage, newUser.Id);
        ...
        public async Task<(bool IsSuccess, string? ErrorMessage, UserDto? UpdatedUser)> UpdateUserById(int id, UserDto dto)
        {
            ...
            if (dto.FileImage != null && dto.FileImage.Length > 0)
            {
                ...
                var imageUrl = await UploadImageAsync(dto.FileImage, id);
                user.Avatar = imageUrl;
            }

            _dbContext.Users.Update(user);
            if (salary != null) _dbContext.Salaries.Update(salary);
            await _dbContext.SaveChangesAsync();
        ...
        private string? GetPublicIdFromUrl(string url)
        {
            try
            {
                ...
                var parts = uri.AbsolutePath.Split('/');
                if (parts.Length < 2) return null;
                var filename = Path.GetFileNameWithoutExtension(parts[^1]); // Loại bỏ phần mở rộng
                return $"avatars/{filename}";
            }
            catch
            {
        ...
        private async Task<string> UploadImageAsync(IFormFile file, int userId)
        {
            if (_cloudinary == null)
                throw new Exception("Cloudinary service not initialized");

            var timestamp = DateTimeOffset.UtcNow.ToUnixTimeSeconds().ToString();
            var signature = GenerateSignature(timestamp, "MDrsLyfVWrjRZ0O1MH83weJFyao"); // ApiSecret

            using var stream = file.OpenReadStream();
            // Inside the UploadImageAsync method:
            var uploadParams = new ImageUploadParams
            {
                ...
                Transformation = new Transformation()
                    .Width(500).Height(500).Crop("fill").Gravity("auto").Quality("auto"),
                EagerTransforms = new List<Transformation>
                {
                    new Transformation().Width(100).Height(100).Crop("fill").Gravity("auto").Quality("auto"),
                    new Transformation().Width(500).Height(500).Crop("fill").Gravity("auto").Quality("auto")
                },
        ...
        private string GenerateSignature(string timestamp, string apiSecret)
        {
            var toSign = $"timestamp={timestamp}{apiSecret}";
            using var sha = SHA1.Create();
            var hash = sha.ComputeHash(Encoding.UTF8.GetBytes(toSign));
